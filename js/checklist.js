import { setAllChecklist, setChecklistPage, CHECKLIST_PER_PAGE } from '/js/state.js';
...
export async function loadSiteChecklist(siteId) {
  ...
  const { allChecklist } = await import('/js/state.js');
  renderChecklist(allChecklist);
}

export async function renderChecklist(items) {
  const { checklistPage } = await import('/js/state.js');
  const container = document.getElementById('checklist-container');
  if (!items.length) {
    container.innerHTML = `<div class="empty"><div class="empty-icon">✓</div><div class="empty-title">No checklist items</div><div class="empty-sub">Run an audit to generate improvements.</div></div>`;
    updateChecklistProgress(); return;
  }
  ...
  const groups = {};
  pageItems.forEach(item => { const g = item.category || 'General'; if (!groups[g]) groups[g]=[]; groups[g].push(item); });
  container.innerHTML = Object.entries(groups).map(([group, groupItems]) => `
    <div class="checklist-group">
      <div class="checklist-group-title">${group}</div>
      ${groupItems.map(item => `
        <div class="checklist-item ${item.done?'done':''} ${item.ignored?'ignored':''}" id="ci-${item.id}" style="${item.ignored?'opacity:.35':''}">
          ...
          <button class="action-btn ${item.ignored?'':'danger'}" onclick="toggleIgnoreItem('${item.id}',${!item.ignored})" style="flex-shrink:0;font-size:.6rem;padding:.25rem .55rem">${item.ignored?'Unignore':'Ignore'}</button>
        </div>`).join('')}
    </div>`).join('');
  if (totalPages > 1) {
    container.insertAdjacentHTML('beforeend', `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:1rem;padding-top:1rem;border-top:1px solid var(--b3)">
        <div style="font-size:.65rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--g3)">Showing ${start+1}–${Math.min(start+CHECKLIST_PER_PAGE,items.length)} of ${items.length}</div>
        <div style="display:flex;align-items:center;gap:.4rem">
          ...
          ${Array.from({length:totalPages},(_,i)=>i+1).filter(p=>p===1||p===totalPages||Math.abs(p-curPage)<=2)
            .reduce((acc,p,i,arr)=>{if(i>0&&p-arr[i-1]>1)acc.push('…');acc.push(p);return acc;},[])
            .map(p=>p==='…'?`<span style="font-size:.65rem;color:var(--g2);padding:0 .25rem">…</span>`
              :`<button class="action-btn" onclick="goChecklistPage(${p})" style="${p===curPage?'border-color:var(--red);color:var(--red)':''}">${p}</button>`).join('')}
...
export async function updateChecklistProgress() {
  const { allChecklist } = await import('/js/state.js');
  const total = allChecklist.length, done = allChecklist.filter(c=>c.done).length;
  document.getElementById('checklist-progress-label').textContent = total ? `${done} of ${total} complete — ${total-done} remaining` : 'No items';
}

export async function goChecklistPage(page) {
...
export async function filterChecklist() {
  setChecklistPage(1);
  const { allChecklist } = await import('/js/state.js');
  const f = document.getElementById('checklist-filter').value;
  let filtered = [...allChecklist];
...
export async function toggleChecklistItem(itemId) {
  ...
  if (!item) return;
  item.done = !item.done; item.completed_at = item.done ? new Date().toISOString() : null;
  const el = document.getElementById('ci-'+itemId);
  if (el) el.classList.toggle('done', item.done);
  updateChecklistProgress();
  await apiFetch('/admin/checklist/'+itemId, { method:'PATCH', body: JSON.stringify({ done: item.done }) });
}
...
export async function toggleIgnoreItem(itemId, ignored) {
  ...
  await apiFetch('/admin/checklist/'+itemId+'/ignore', { method:'PATCH', body: JSON.stringify({ ignored }) });
  toast(ignored ? "Item ignored — won't reappear after next audit" : 'Item unignored');
}

export async function clearCompletedTasks() {
  const { allChecklist, currentSiteId } = await import('/js/state.js');
  ...
  const { allChecklist: fresh } = await import('/js/state.js');
