...
export function renderSiteKeywords() {
  ...
  if (ungrouped.length) {
    if (allKeywordGroups.length) html += `<tr><td colspan="7" style="padding:.5rem .85rem;background:var(--b2);font-size:.6rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--g2);border-bottom:1px solid var(--b3)">Ungrouped (${ungrouped.length})</td></tr>`;
...
export async function addKeyword() {
  ...
  const res = await apiFetch('/admin/sites/' + currentSiteId + '/keywords', {
    method: 'POST',
...
export async function addKeywordGroup() {
  ...
  const res = await apiFetch('/admin/sites/' + currentSiteId + '/keyword-groups', {
    method: 'POST',
...
export function renderGroupPills() {
  ...
  if (!container) return;
  container.innerHTML = allKeywordGroups.map(g => `
