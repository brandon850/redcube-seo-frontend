...
export function populateSiteDropdowns() {
  const { sites } = window.__state || {};
  // Use dynamic import to get fresh state
  import('/js/state.js').then(({ sites }) => {
    ['kw-site-filter','content-site-filter','report-site-filter','editor-site'].forEach(id => {
      ...
      sites.forEach(s => { const o = document.createElement('option'); o.value = s.id; o.textContent = s.name; el.appendChild(o); });
      el.value = saved;
    });
  });
}
...
export async function renderOverview() {
  ...
  const openTasks = sites.reduce((n, s) => n + (s.open_tasks || 0), 0);
  const avgScore  = sites.length ? Math.round(sites.reduce((n, s) => n + (s.last_score || 0), 0) / sites.length) : 0;
  ...
  document.getElementById('stat-kw').textContent      = sites.reduce((n, s) => n + (s.keyword_count || 0), 0);
  document.getElementById('stat-drafts').textContent  = sites.reduce((n, s) => n + (s.draft_count || 0), 0);
  renderSitesTable('overview-sites-tbody', true);
}

export async function renderSitesList() {
  document.getElementById('sites-list-view').style.display  = '';
  document.getElementById('site-detail-view').style.display = 'none';
  renderSitesTable('sites-tbody', false);
}

export async function renderSitesTable(tbodyId, isOverview) {
  const { sites } = await import('/js/state.js');
  const tbody = document.getElementById(tbodyId);
  if (!tbody) return;
  if (!sites.length) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:3rem;color:var(--g2);font-size:.75rem;letter-spacing:.06em;text-transform:uppercase">No sites yet — add your first client site</td></tr>`;
    return;
  }
  tbody.innerHTML = sites.map(s => {
    const gc = gradeColor(s.last_grade || '—');
    return `<tr>
      <td><div class="site-row-name">${s.name}</div><div class="site-row-url">${s.url}</div></td>
...
export async function openSiteDetail(siteId) {
  ...
  await Promise.all([loadSitePages(siteId), loadSiteChecklist(siteId), loadSiteKeywords(siteId), loadSiteContent(siteId)]);
}
...
export async function runFullAudit() {
  ...
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Auditing...'; wrap.style.display = '';
  const steps = [{p:5,m:'Connecting...'},{p:15,m:'Crawling pages...'},{p:30,m:'Analyzing titles...'},{p:45,m:'Checking content...'},{p:58,m:'Verifying mobile...'},{p:70,m:'Evaluating speed...'},{p:82,m:'Checking schema...'},{p:91,m:'Generating checklist...'},{p:97,m:'Saving results...'}];
  let si = 0;
  const tick = setInterval(() => { if (si < steps.length) { fill.style.width = steps[si].p + '%'; msg.textContent = steps[si].m; si++; } }, 3000);
  try {
    const res = await apiFetch('/admin/sites/' + currentSiteId + '/audit', { method: 'POST' });
    clearInterval(tick); fill.style.width = '100%'; msg.textContent = 'Audit complete!';
    if (!res || !res.ok) { const e = await res?.json().catch(()=>({})); throw new Error(e.error || 'Audit failed'); }
    const data = await res.json();
    await loadSites();
    const { sites: freshSites } = await import('/js/state.js');
    const updated = freshSites.find(s => s.id === currentSiteId);
    if (updated) {
    ...
    const { loadSitePages }     = await import('/js/pages.js');
    const { loadSiteChecklist } = await import('/js/checklist.js');
    await loadSitePages(currentSiteId); await loadSiteChecklist(currentSiteId);
    toast('Audit complete — ' + (data.pages_crawled || 0) + ' pages crawled');
    setTimeout(() => { wrap.style.display = 'none'; fill.style.width = '0%'; }, 2000);
  } catch (err) {
    clearInterval(tick); toast('Audit failed: ' + err.message); wrap.style.display = 'none';
  } finally { btn.disabled = false; btn.innerHTML = '▶ Run Audit'; }
}

export async function saveMaxPages(val) {
  const { currentSiteId, sites } = await import('/js/state.js');
  if (!currentSiteId) return;
  await apiFetch('/admin/sites/' + currentSiteId + '/settings', { method:'PATCH', body: JSON.stringify({ max_pages: parseInt(val) }) });
  const site = sites.find(s => s.id === currentSiteId);
...
export async function addSite() {
  ...
  const res = await apiFetch('/admin/sites', { method:'POST', body: JSON.stringify({ name, url, platform, client_email: email, notes }) });
  if (!res || !res.ok) { toast('Failed to add site'); return; }
  closeModal('modal-add-site');
  await loadSites(); renderSitesList(); renderOverview();
...
export async function deleteSite(siteId, name) {
  ...
  renderSitesList(); renderOverview();
  toast(name + ' removed');
}
