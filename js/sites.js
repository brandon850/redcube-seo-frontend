...
export function renderOverview() {
  ...
  const avgScore  = sites.length
    ? Math.round(sites.reduce((n, s) => n + (s.last_score || 0), 0) / sites.length) : 0;
...
export async function runFullAudit() {
  ...
  try {
    ...
    const data = await res.json();
    await loadSites();
    const updated = sites.find(s => s.id === currentSiteId);
    if (updated) {
...
export async function saveMaxPages(val) {
  if (!currentSiteId) return;
  await apiFetch('/admin/sites/' + currentSiteId + '/settings', {
    method: 'PATCH',
