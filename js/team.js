...
export async function loadTeam() {
  ...
  if (!members.length) {
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--g2);font-size:.72rem;text-transform:uppercase;letter-spacing:.06em">No team members yet</td></tr>`;
    return;
  }
  tbody.innerHTML = members.map(m => `
    <tr>
      <td style="font-weight:600;color:var(--white)">${m.name||'—'}</td>
      <td style="color:var(--g4)">${m.email}</td>
      <td><span class="tag">${m.role}</span></td>
      <td style="color:var(--g4)">${relativeTime(m.last_login)}</td>
      <td><button class="action-btn danger" onclick="removeMember('${m.id}')">Remove</button></td>
    </tr>`).join('');
}
